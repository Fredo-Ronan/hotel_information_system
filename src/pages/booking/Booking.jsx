...
export const BookingPage = () => {
  ...
  // METODE PEMBAYARAN BOOLEAN HANDLER
  const [isKartuKredit, setIsKartuKredit] = useState(true);
  const [isTransferBank, setisTransferBank] = useState(false);

  const metodePembayaranHandler = (what) => {
    if(what === "Kartu Kredit") {
      setIsKartuKredit(true);
      setisTransferBank(false);
    } else if(what === "Transfer Bank") {
      setisTransferBank(true);
      setIsKartuKredit(false);
    }

    setMetodePembayaran(what);
  }
  ...
  const submitHandler = (event) => {
    ...
    console.log(data);
    sessionStorage.setItem("detailBooking", JSON.stringify(data));
    navigate("/user/rooms/booking/payment");
  };
                ...
                <Card.Body className="p-2 m-0">
                  <Tab.Container id="myTab" defaultActiveKey="tab1">
                    <Nav variant="tabs" className="nav nav-tabs">
                      <Nav.Item>
                        <Nav.Link eventKey="tab1" onClick={() => metodePembayaranHandler("Kartu Kredit")}>Kartu Kredit</Nav.Link>
                      </Nav.Item>
                      <Nav.Item>
                        <Nav.Link eventKey="tab2" onClick={() => metodePembayaranHandler("Transfer Bank")}>Transfer Bank</Nav.Link>
                      </Nav.Item>
                    </Nav>
